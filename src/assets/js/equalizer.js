...
function _drawTooltip(tooltipModel, dom) {
  ...
  ttDom.style.left =
    position.left + window.pageXOffset + tooltipModel.caretX - shiftLeft + 'px';
  ttDom.style.top = position.top + window.pageYOffset + realY - shiftUp + 'px';
}
class Equalizer {
  constructor(canvasPref, eqConf) {
    this.debugMode = false;
    this.mainCtx = document
      .getElementById(canvasPref + 'Canvas')
      .getContext('2d');
    this.customCtx = document
      .getElementById(canvasPref + 'Custom')
      .getContext('2d');
    this.shadowCtx = document
      .getElementById(canvasPref + 'Shadow')
      .getContext('2d');
    ...
    this.chartHeight =
      ...
      this.option.topOffset -
      this.option.bottomOffset;
  ...
  printData() {
    if (this.debugMode) {
      ...
      for (var i = 0; i < 10; i++) {
        document.getElementsByClassName('eq-data-' + i)[0].innerText =
          data[i] + ' dB';
      }
    }
  }
  ...
  updateTooltip(e, newValue) {
    ...
    ttDom.style.left =
      this.baseLeft +
      this.selectedPoint._index * this.option.gridSpace -
      shiftLeft +
      'px';
    ...
    let realY = 306 - 12 * i - Math.floor((i + 1) / 3);
    ttDom.style.top =
      position.top + window.pageYOffset + realY - shiftUp + 'px';
  }
  updatePoint(i = 0, newValue = 0) {
    let obj = this.chartObj;

    obj.data.datasets[0].pointBackgroundColor[i] = this.option.pointClickColor;
    obj.data.datasets[0].pointHoverBackgroundColor[i] =
      this.option.pointClickColor;
  ...
  handleMouseDown(e) {
    if (this.chartObj != null) {
      ...
      if (this.selectedPoint) {
        let obj = this.chartObj;
        obj.data.datasets[0].pointBackgroundColor[this.selectedPoint._index] =
          this.option.pointClickColor;
        obj.data.datasets[0].pointHoverBackgroundColor[
          this.selectedPoint._index
        ] = this.option.pointClickColor;
